...
// EasyCAT basic example using DC syncronization ---------------------------------------------------
//
// In this example the INT signal coming from the LAN9252 generates an interrupt 
...
EasyCAT EASYCAT(DC_SYNC);           // EasyCAT istantiation
                                    // In order to use the DC Synchronization we have to set 
                                    // the "DC_SYNC" parameter in the constructor 
                                    ...
                                    // example:                                  
//EasyCAT EASYCAT(8, DC_SYNC);      // pin 8 will be used as SPI chip select
...
const byte InterruptPin = 2;        // This pin receive the INT
...
void setup()
{
  ...
  pinMode(InterruptPin, INPUT);                             // interrupt from LAN9252
  ...
  {                                                         // the EasyCAT board was not recognized
    ...
    {                                                       //   
      ...
      delay(500);                                           // 
                                                            ...
                                                            // INT pin setting
    attachInterrupt(digitalPinToInterrupt(InterruptPin), Campionatura, FALLING);  
    
    //attachInterrupt(InterruptPin, Campionatura, FALLING); //  !!!!! use this for Arduino 101   !!!!!!   
