...
{
    periodInfo->nextPeriod.tv_nsec += periodInfo->periodNs; //Incrementing the NextPeriod interval for sleeping, the timer source is monotonic
    while (periodInfo->nextPeriod.tv_nsec >= nSecondsInSeconds) { //tv_nsec has to be less than nSecondsInSEconds, because seconds are dealt with in tv_sec
...
uint8_t EthercatMaster::WaitUntilPeriodElapsed(PeriodInfo *periodInfo)
{
    ...
    uint8_t msg = clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &periodInfo->nextPeriod, NULL); //Just Sleep until the end of the time period required
    if (msg !=0) {
...
void *EthercatMaster::TheRtThread(void *args)
{
    ...
    thisInstance->SetSchedulerParameters(thisInstance->masterData.rtCpuId,thisInstance->masterData.rtPriority);
    ...
    while (cycleFlag) {
        pthread_mutex_lock( &thisInstance->rtMutex ); // Lock the resources while we are using it
        ...
        if (!thisInstance->flags.notSync && thisInstance->flags.configState && thisInstance->flags.masterState && thisInstance->flags.domainState) {
                thisInstance->LoopFunction(); // If everything is in order, execute loopfunction of master
        } else {
...
void EthercatMaster::CheckDomainState() // Check ethercat domain state machine
{
    ...
    if (domainStateLocal.wc_state != domainState.wc_state) {
        ...
        if (domainStateLocal.wc_state == domainOperational) flags.domainState = operationalState;
        else flags.domainState = notOperationalState;
    }
...
void EthercatMaster::CheckMasterState() // Check ethercat master state machine
{
    ...
    if (masterStateLocal.al_states != masterState.al_states) {
        ...
        if (masterStateLocal.al_states == masterOperational) flags.masterState = operationalState;
        else flags.masterState = notOperationalState;
    }
...
void EthercatMaster::CheckConfigState() // Check ethercat slave configuration state machine
{
    ...
    if (configStateLocal.al_state != configState.al_state) {
        ...
        if (configStateLocal.al_state == slaveOperational) flags.configState = operationalState;
        else flags.configState = notOperationalState;
    }
...
void EthercatMaster::SetSchedulerParameters(uint8_t threadCpu, uint8_t threadPriority) // Thread utility..
{
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
uint8_t EthercatMaster::InitProtocol()
{
    ...
    for (int i=0;i<numberOfSlaves;i++) { // Configuring Slaves
        if (!(configPointer = ecrt_master_slave_config(masterPointer,slave[i]->alias,slave[i]->position,slave[i]->vendor_id,slave[i]->product_code))) {
    ...
    if (ecrt_master_activate(masterPointer)) { // Activating Master
        cout << "Error activating Master" << endl;
        return notValidate;
    }
    if (!(domainDataPointer = ecrt_domain_data(domainPointer))) { // Activating Domain
        cout << "Error Initializing Domain Data" << endl;
        return notValidate;
    }

    for (int i=0;i<numberOfSlaves;i++) slave[i]->Init(domainDataPointer); // Performing Slave initialization function, default is empty function
...
{
    thisInstance = this; // we need the address of the actual master, we'll need it in the static functions
}
...
void EthercatMaster::Start()
{
    ...
    if (pthread_attr_init(&threadAttributes)) {
        ...
        if (pthread_attr_setstacksize(&threadAttributes, PTHREAD_STACK_MIN + thisStackSize)) {
            ...
            if (thisInstance->InitProtocol()){ // If everything is ok, start the master thread on a differrent thread
                SetSchedulerParameters(masterData.guiCpuId,masterData.guiPriority); // Set priority for gui thread
                pthread_create(&threadRt, &threadAttributes, TheRtThread, NULL); // master thread start
            }
            else cout << "Could not initialize Ethercat Devices" << endl;
        }
    }
}
